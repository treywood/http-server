...
route Req.POST ["api", "path"] req =
  let method' = show req.method
   ...
   in respond $
      ...
      method' ++
      " request to " ++
      req.path ++ " with headers: " ++ headers' ++ " and body: " ++ body'
route Req.GET ["api", "greet", name] req =
  let it =
        if name == "kyle"
          then Nothing
          else Just ("hello " ++ name)
   in respond it
...
route Req.POST ["api", "json"] req =
  case Req.json req of
    ...
    Left err -> respond (400, "Bad JSON: " ++ err)
route Req.GET ("assets":path) _ = static "target" (intercalate "/" path)
route Req.GET ["html"] _ =
  respond $
  H.html
    []
    [ H.head [] [H.title [] [H.text "TEST"]]
    , H.body
        []
      ...
      Right json ->
        case get "timezone" json of
          Just (JsonString timezone) ->
            return $
            H.html [] [H.div [] [H.text $ "Your timezone is " ++ timezone]]
          _ -> return $ H.html [] [H.div [] [H.text "Your timezone is missing"]]
      Left error -> return $ H.html [] [H.div [] [H.text $ "ERROR: " ++ error]]
route Req.GET _ _ = sendFile "target/index.html"
route _ _ _ = respond notFoundResponse
