...
parseHeaders = parseHeaders' []
  where
    ...
    parseHeaders' hs = do
      ...
      if List.null line
        then return hs
        else let name = takeWhile (/= ':') line
                 value = List.drop 2 $ dropWhile (/= ':') line
                 header = (name, value)
              in parseHeaders' (header : hs)
